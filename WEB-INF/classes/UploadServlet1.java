...
saveFile= vid+saveFile.substring(saveFile.lastIndexOf("."));
String zz=".//webapps//smartcity//images//vehi//"+saveFile;
String zzz=".//webapps//smartcity//images//vehi//";
/* saveFile holding the image/video filename, now get the index of . from savefile -> use this value  to substring from savefile to get extention. then attach the vname, imgname from database table with the extention. now this new file name must be stored in saveFile and  in database*/
    ...
    try{
        ...
        rs = st.executeQuery("select * from vehicle where vid='"+vid+"'");
        try{
            if(rs.next()){
	...
	st.executeUpdate("update vehicle set photo='"+saveFile+"'   where vid='"+vid+"'");	
           } 
       }catch(Exception eee){ } 
    } catch(Exception e){
        e.printStackTrace();
  ...
  RequestDispatcher rd1= sc.getRequestDispatcher("/vehicle-signup5.jsp"); 
